...
Dev! : @iazuh''')
                ...
                # تنسيق البطاقة قبل فحصها
                cc_formatted = reg(cc)
                if not cc_formatted:
                    print(f"Skipping invalid card: {cc.strip()}")
                    continue

                try:
                    start_time = time.time()
                    # تصحيح استدعاء الدالة
                    last = str(UniversalStripeChecker(cc_formatted))
                except Exception as e:
                    print(e)
                    last = "ERROR"
                mes = types.InlineKeyboardMarkup(row_width=1)
                cm1 = types.InlineKeyboardButton(f"• {cc_formatted} •", callback_data='u8')
...
- Time: {execution_time:.2f}s''',
                ...
                n = cc_formatted.split("|")[0]
                mm = cc_formatted.split("|")[1]
                yy = cc_formatted.split("|")[2]
                cvc = cc_formatted.split("|")[3].strip()

                cc_final = n+'|'+mm+'|'+yy+'|'+cvc
                msg = f'''<strong>#Brantree_LookUp_(Passed) 🔥
- - - - - - - - - - - - - - - - - - - - - - -
[<a href="https://t.me/B">ϟ</a>] 𝐂𝐚𝐫𝐝: <code>{cc_final}</code>
